...
const customerRegistration = async (req, res) => {
  ...
  try {
    const Customer = await customerModel.create({
      ...
      customerImage: "https://cdn-icons-png.flaticon.com/512/149/149071.png",
    });
    res
      .status(200)
      .send({ msg: "Customer registered Successfully", customer: Customer });
  } catch (error) {
    console.log(error);
...
const orderData = async (req, res) => {
  const { cusid } = req.body;
  try {
    const Customer = await shippingModel
      .findOne({ cusid: cusid })
      .sort({ createdAt: -1 });
    if (!Customer) {
...
const orderDetail = async (req, res) => {
  const { cusid } = req.body;
  try {
    const Order = await OrderModel.findOne({ Customer: cusid }).sort({
      orderAt: -1,
    });
    res.status(200).send(Order);
  } catch (error) {
...
const orderHistory = async (req, res) => {
  ...
  try {
    const Order = await OrderModel.find({ Customer: cusid }).sort({
      orderAt: -1,
    });
    res.status(200).send(Order);
  } catch (error) {
...
const profileUpdateData = async (req, res) => {
  ...
  try {
    const updatedFields = { name, email, number, address, city, state };

    if (profilePicture) {
      updatedFields.customerImage = profilePicture;
    }
